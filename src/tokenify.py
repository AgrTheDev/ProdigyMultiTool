...
def tokenify(username: str, password: str) -> TokenifyOutput:
    ...
    if not login.ok and not str(login.status_code).startswith("3"):
        throwerror(f"Initial login request was unsuccessful with code {login.status_code}.")
    ...
    if schoolLogin.status_code != 200:
        throwerror('There was an error while trying to login to prodigy!\nCheck your credentials and if this error persists, join our discord server for support.\nhttps://discord.gg/HDntGJAK6c')
    ...
    if not tokenLogin.ok and not str(tokenLogin.status_code).startswith("3"):
        throwerror(f"First authentication request failed with a code of {tokenLogin.status_code}.")
    ...
    if not secondTokenLogin.ok and not str(secondTokenLogin.status_code).startswith("3"):
        throwerror(f"Second authentication request failed with a code of {secondTokenLogin.status_code}.")
    ...
    if not master.ok:
        throwerror(f"Master request failed with a code of {master.status_code}.")

    masterJSON = master.json()
    return masterJSON['token']